...
signal1 = longSignal(0*samplesPerCode + [1 : samplesPerCode]);
...
for PRN = settings.acqSatelliteList
    ...
    for frqBinIndex = 1:numberOfFrqBins
        ...
        % #################################################################
        % generat 1ms replica of PRN code
        prn_code = generateGoldCodeSampled(PRN, settings.samplingFreq, settings.codeFreqBasis, 1);
        ...
        r = ifft(fft(signal1.*(c1+1i*c2)).*conj(fft(prn_code)));

        % Store correlation results ------------
        results(frqBinIndex, :) = r .* conj(r);
